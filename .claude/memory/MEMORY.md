...
- 2026-05-09–12: Major Marlow buildout. Cron→launchd migration (Keychain auth bug). News curation pipeline (candidate notes during scans → end-of-day cross-source curation → Telegram). Memory grader (`grade_memory` daily). Drafting pipeline (`draft_article` weekly + `marlow draft <slug>` on-demand). Inspection CLI (`marlow threads/notes/memory/news`). Astro blog site at `projects/blog/site/` — dark mode default, content collections, RSS, 6 routes. Marlow produced first autonomous news digest (5/9 picks, Mythos+RSI anchors) and first draft ("Three data points in a week, one asymmetry"). She filed three architectural concerns of her own via working.md → I fixed all three (scheduler eager-fire + dedup, UTC rollover, channel-split clarification). Also built a parallel **Simona daemon** in this repo: `daemon/` mirroring Marlow's framework, LaunchAgent `com.simona.tick` (2h interval), tasks `review_drafts` + `observe_marlow`, on-demand `simona review`/`simona observe`. First observation file at `memory/observations/marlow/2026-05-12.md` is genuinely meta — caught Marlow's AI-essay tells creeping in, flagged Anthropic-orbit selection bias in the RSI thread, identified coverage gaps (DeepMind/OpenAI feeds, governance feed). Full development arc: `~/projects/marlow/DEVLOG.md` (created the convention this session — every framework-changing session appends an entry, see CLAUDE.md "Marlow project — devlog discipline"). Marlow committed + pushed through `916b7fa`.
