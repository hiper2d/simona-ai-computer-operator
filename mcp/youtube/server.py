...
# Create an MCP server (stdio transport for direct Claude Code integration)
mcp = FastMCP(
...
def get_youtube_transcript(
    ...
    Args:
        ...
        format: Output format — "text" for plain text (compact, best for analysis/summary),
                "segments" for full JSON with timestamps (larger, use for precise time references).
                Default: "text"
        start_time: Only include segments starting at or after this time in seconds (default: 0 = start)
        end_time: Only include segments up to this time in seconds (default: 0 = entire video)

    Returns:
        If format="text": Plain transcript text with paragraph breaks every ~60 seconds.
            Prefixed with metadata (segment count, duration range).
    ...
    for seg in segments:
        ...
        # Break into paragraphs roughly every 60 seconds
...
def find_code_segments(
    ...
    Uses keyword heuristics with a sliding window approach to detect segments
    where the speaker is likely showing or discussing code.
    ...
    Returns:
        JSON string with list of detected code segments, each containing
        'start_time', 'end_time', 'confidence' (0-1), and 'keywords_found'.
    """
    # Get transcript first
    transcript_json = get_youtube_transcript(video_url, language)
    ...
    # Merge overlapping segments with similar confidence
    merged = []
    for seg in code_segments:
        if merged and seg["start_time"] <= merged[-1]["end_time"]:
            # Merge: extend end time, combine keywords, take max confidence
            prev = merged[-1]
...
def get_video_frames(
    ...
    Downloads the video at lowest quality (to minimize bandwidth), then uses
    ffmpeg to extract frames at the specified rate.
    ...
    Returns:
        JSON string with list of extracted frame file paths (absolute paths).
        Use Claude's Read tool to view the frame images.
    """
    video_id = extract_video_id(video_url)
    cache_path = get_cache_path(video_id)
    ...
    try:
        ...
        return json.dumps({
            "error": f"Failed to extract frames: {e.stderr}",
        })
...
def cleanup_cache(
    ...
    Args:
        ...
        delete_all: If True, delete everything in the cache regardless of age
